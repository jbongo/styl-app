...
@endmandataire
@intervenant
    Les Formations que j' ai animé
@endintervenant
@formateur
    Les Formations que j' ai suppervisé
@endformateur
@admin
    Formations passées
@endadmin
      ...
      <div class="card-body">
        <div class="table-responsive" style="overflow-x: inherit !important;">
            <table  id="formations" class=" table student-data-table  m-t-20 "  style="width:100%">
                ...
                @if (strtotime($formation->starting_date) < time() && !($formation->is_model))
                  <tr>
                      <td width="13%">
                      ...
                      <td width="3%"> @admin @if ($formation->nb_inscrits) <a id="buttonusr" data-toggle="modal" style="font-size: small;" title="Inscrits" data-target="#usr-{{ $formation->id }}" href="#users"><strong class="badge badge-grey hoverusrs">{{ $formation->nb_inscrits }}</strong></a> @endif @endadmin
                      </td>
                      ...
                      <td>
                          <span>
                              @if ( $formation->multiname )<a href="#addmulti-{{ $formation->id }}" class="retrievemulti" id="rmulti-{{ $formation->id }}" data-toggle="modal" title="replanifier la formation multiple"><i class="material-icons color-darkgreen hovergray multi modify_modal_trigger" transfer="{{ $formation->id }}">format_list_numbered</i></a> @endif
                              <a  href="storage/formationDL/{{ $formation->id }}/{{ Auth::user()->role }}" id="{{ $formation->id }}" data-toggle="tooltip" title="télecharger les documents de ma formation">
                                  <i class="material-icons color-info hovergray">
                                      attachment
                                  </i>
                              </a>
                              <a  href="/formation/delete/{{ $formation->id }}" class="delete" id="{{ $formation->id }}" data-toggle="tooltip" title="archiver"><i class="material-icons color-danger hovergray">delete</i></a>
                          </span>
                      </td>
                  </tr>
                @endif
                ...
                @if (strtotime($formation->starting_date) < time() && !($formation->is_model) && $usr->id === Auth::user()->id)
                  <tr>
                      <td width="13%">
                      <span style="display: none;"> {{ $formation->starting_date }} </span>{{ strftime("%a %d %B", strtotime($formation->starting_date)) }} <strong @if (intval(strftime("%H", strtotime($formation->starting_date))) < 12) style="color: #ddaa00;" @else style="color: darkblue;" @endif>{{ strftime("%H:%M", strtotime($formation->starting_date)) }}</strong>
                      </td>
                ...
                @formateur
                @foreach ($formations as $formation)
                @if (strtotime($formation->starting_date) < time() && !($formation->is_model) && ($formation->former_id === Auth::user()->id || $formation->speaker_id === Auth::user()->id))
                  <tr>
                      <td width="13%">
                      <span style="display: none;"> {{ $formation->starting_date }} </span>{{ strftime("%a %d %B", strtotime($formation->starting_date)) }} <strong @if (intval(strftime("%H", strtotime($formation->starting_date))) < 12) style="color: #ddaa00;" @else style="color: darkblue;" @endif>{{ strftime("%H:%M", strtotime($formation->starting_date)) }}</strong>
                      </td>
                ...
                @intervenant
                @foreach ($formations as $formation)
                @if (strtotime($formation->starting_date) < time() && !($formation->is_model) && $formation->speaker_id === Auth::user()->id)
                  <tr>
                      <td width="13%">
                      <span style="display: none;"> {{ $formation->starting_date }} </span>{{ strftime("%a %d %B", strtotime($formation->starting_date)) }} <strong @if (intval(strftime("%H", strtotime($formation->starting_date))) < 12) style="color: #ddaa00;" @else style="color: darkblue;" @endif>{{ strftime("%H:%M", strtotime($formation->starting_date)) }}</strong>
                      </td>
...
<script>
 ...
 $(".retrievemulti").click(function(){
  $.multifunction = function(){
      ...
      $.ajax({
         type : "GET",
         url: '/formation/multireplan/' + transfer,
         dataType: 'json',
 ...
 $('a.uncat').click(function(e) {
    ...
    <script>
        ...
        swalWithBootstrapButtons({
            title: '@lang('Vraiment ARCHIVER cette formation ? (sans le statut administrateur vous perdez accès à cette formation)')',
            type: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#DD6B55',
            confirmButtonText: '@lang('Oui')',
