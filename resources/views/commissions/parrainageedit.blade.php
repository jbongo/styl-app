...
*@description : formulaire de modification d'un model de parrainage
*/
      ...
      <div class="card">
         ...
         <div class="card-body">
            <div class="form-validation">
               <form class="form-valide3" action="{{ route('parrainage.update', $pln->id) }}" method="post">
                  {{ csrf_field() }}
                  <div class="form-group row">
                     <div class="col-lg-4">
                        <label class="col-lg-3 col-form-label" for="plan-name">Nom du plan <span class="text-danger">*</span></label>
                     <input type="text"  class="form-control" id="plan-name" name="plan-name" value="{{$pln->nom}}" required>
                     </div>
                  </div>
...
<script>
   $('.submit').click(function(e){ 
      ...
      if (form.valid() == true){
        ...
        $.ajax({
            type: "POST",
            url: ("{{ route('parrainage.update', $pln->id) }}"),
